...
(function () {
  ...
  const drawChart = parsedData => {
      ...
      const svg = select('body').append('svg').attr('width', svgWidth).attr('height', svgHeight).on('wheel', function () {
      ...
      function zoomed() {
          ...
          selectAll(".boundary").style("stroke-width", 1 / s);
          selectAll('.point').attr('r', 0.01 / s);
      ...
      function zoomended() {
          if (s !== 1) return;
          rotated = rotated + ((mouse$1[0] - initX) * 360 / (s * svgWidth));
          mouseClicked = false;
      }
